"""
panel.py — FRANZ transparent reverse proxy, dashboard, and logger.

Sits between main.py and the upstream VLM (LM Studio at :1235), forwarding
all traffic byte-for-byte while observing, verifying SST, saving screenshots,
and streaming turn data to a live HTML dashboard via SSE.

Auto-launches main.py after a 10-second startup delay. All pipeline logging
(batched JSON + per-turn PNGs) is centralized here. On Ctrl-C, flushes
remaining logs and terminates main.py gracefully.
"""
...
def _verify_sst(turn: int, sst: str) -> dict:
    prev = _get_last_vlm()
    r: dict = {"verified": False, "match": False, "prev_available": prev is not None, "detail": ""}
    if prev is None:
        r["verified"] = r["match"] = True
        r["detail"] = "Turn 1: no previous response" if sst == "" else "First observed turn"
        return r
    r["verified"] = True
    if sst == prev:
        r["match"] = True
...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        si = "✓" if sst.get("match") else "✗ VIOLATION"
        sys.stdout.write(
